...
#define IsLChild(x) (!IsRoot(x) && (&(x) == (x).parent->lc))//是否左儿子
#define IsRChild(x) (!IsRoot(x) && (&(x) == (x).parent->rc))//是否右儿子
#define HasParent(x) (!IsRoot(x))//有没有父亲
#define HasLChild(x) ((x).lc)//有没有左儿子
#define HasRChild(x) ((x).rc)//有没有右儿子
...
public:
	...
	BinNodePosi(T)lc;//左儿子
	BinNodePosi(T)rc;//右儿子
	...
	BinNode(T const &e, BinNodePosi(T) par = nullptr, BinNodePosi(T) lc = nullptr, BinNodePosi(T) rc = nullptr, int hei = 0, int l = 0, RBColor c = RB_RED)
		:data(e), parent(par), lc(lc), rc(rc), height(hei), npl(l), color(c) {}
...
template<class VST> void BinNode<T>::travLeveL(VST &visit)//子树层次遍历
{
	...
	while (!Q.empty())//队列清空前循环
	{
		...
		visit(t->data); //并访问
		if (HasLChild)Q.enqueue(t->lc);//若有左孩子，则入队
		if (HasRChild)Q.enqueue(t->rc);//若有右孩子，则入队
	}
}
...
template<class VST> void BinNode<T>::travPre_1(BinNodePosi(T) t,VST &visit)//子树先序遍历#迭代版本1
{
	...
	while (!s.empty())
	{
		t = s.pop();
		visit(t->data);
		if (HasRChild(*t))s.push(t->rc);//先把右儿子入栈，先进后出
		if (HasLChild(*t))s.push(t->lc);//再把左儿子入栈，后进先出
	}
}
template<class T>
...
template<class VST> void BinNode<T>::travPre_R(BinNodePosi(T) t, VST &visit)//子树先序遍历#递归版本
{
	...
	travPre_R(t->lc, visit);//递归访问左树
	travPre_R(t->rc, visit);//递归访问右树
}
...
{
	if (!t)return;//递归基
	travPost_R(t->lc, visit);
	travPost_R(t->rc, visit);
	visit(t->data);
}
											...
											(BinNodePosi(T) t, VST& visit, Stack<BinNodePosi(T)>& s)
{
	while (t)//当前节点为空时结束循环
	{
		...
		if (HasRChild(*t))s.push(t->rc);//非空右孩子入栈
		t = t->rc;//沿着左分支进入下一层
	}
}
template<class T>
...
static void BinNode<T>::gotoHLVFL(Stack<BinNodePosi(T)> &s)//在以s栈顶节点为根的子树中，寻找最深左孩子叶节点
{
	...
	while (HasChild(*t))//若有孩子
	{
		...
		if (HasRChild(*t))s.push(t->rc);//若有右孩子则优先入栈
		if (HasLChild(*t))s.push(t->lc);//然后判断是否有左孩子，有则入栈
	}//抵达叶节点时退出循环
}
