...
private:
	...
	template<class VST> static void goAlongLeftBranch   //左分支依次入栈，直至无左分支的节点
				(BinNodePosi(T) t, Stack_V<BinNodePosi(T)> &s);
	static void gotoHLVFL(Stack_V<BinNodePosi(T)> &s);//在以s栈顶节点为根的子树中，寻找最深左孩子
};
template<class T>
int BinNode<T>::size()
{
...
template<class T>
static void BinNode<T>::gotoHLVFL(Stack_V<BinNodePosi(T)> &s)//在以s栈顶节点为根的子树中，寻找最深左孩子叶节点
{
	//t = s.top();//拷贝栈顶
